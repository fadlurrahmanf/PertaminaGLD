...
class SerialBridge:
    """Gateway serial monitor and provisioning command transport."""
    ...
    def write_line(self, line: str, require_connected: bool = True) -> dict[str, Any]:
        ...
        if display_line.startswith(("SET_WIFI_CONFIG_JSON ", "SET_MQTT_CONFIG_JSON ")):
            # The actual bytes still go to the selected board, but passwords
            # and TLS material must never be mirrored into SSE/expert logs.
            display_line = f"{display_line.split(' ', 1)[0]} [REDACTED]"
        self._emit("serial_tx", {"line": display_line})
        return {"ok": True}
...
def validate_firmware_package(
    ...
    allowed_top = required_top | {"boardShape", "mqttTransport"}
    ...
    expected_metadata = {
        "gw_small": ("rectangle", "non_tls"),
        "gw_large": ("circle", "non_tls"),
        "gw_small_tls": ("rectangle", "tls"),
        "gw_large_tls": ("circle", "tls"),
    }.get(manifest_env)
    if expected_metadata is not None:
        expected_board_shape, expected_transport = expected_metadata
        if manifest.get("boardShape") != expected_board_shape:
            raise RuntimeError(
                f"manifest boardShape {manifest.get('boardShape')!r} does not match {expected_board_shape!r}"
            )
        if manifest.get("mqttTransport") != expected_transport:
            raise RuntimeError(
                f"manifest mqttTransport {manifest.get('mqttTransport')!r} does not match {expected_transport!r}"
            )
