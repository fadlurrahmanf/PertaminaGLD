# Pertamina GLD Current-Firmware Design Draft - GLD Node
...
Dokumen ini adalah versi penyesuaian GLD design terhadap firmware yang benar
sedang dikompilasi oleh workspace utama. Tujuannya bukan menyalin seluruh
baseline lama, tetapi menyaring baseline tersebut terhadap source firmware
current.
...
| Predicted class | Protocol gas class |
|---:|---|
| 0 | `clearGas` |
| 1 | `LPG` |
| 2 | `methane` |
...
| Accepted command payload | byte 0 `0x01`, byte 1 mode `0|1|2` |
...
| LoRa downlink | Only mode command payload `0x01 + mode` is accepted. |
