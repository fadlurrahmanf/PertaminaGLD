...
assert len(doc.pages)==9
assert len(list(HERE.glob('[0-9][0-9]-diagram.svg')))==9
...
assert not re.search(r'\bU\d+\b|\bkanal\b',text,re.I)
assert not re.search(r'5VEXT|Current not determined|pages 10|/ 17',text,re.I)
assert 'Sensor slot 1 (example)' in text
assert '0,33225 A' in text and '4,66775 A' in text and '7,97 W' in text
assert '6,6449%' in text and '93,3551%' in text
assert all(tuple(map(float,p.mediabox[2:]))==(1200.,800.) for p in doc.pages)
links=sum(len(p.get('/Annots',[])) for p in doc.pages)
from allocation_budget import calculate_allocation
allocation=calculate_allocation()
assert json.loads((HERE/'allocation-calculations.json').read_text())==allocation
assert abs(sum(g['equivalent_24V_A'] for g in allocation['groups'])-allocation['total_A'])<1e-12
assert abs(allocation['remaining_A']+allocation['total_A']-5)<1e-12
