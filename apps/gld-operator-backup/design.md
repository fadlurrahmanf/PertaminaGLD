...
### 12.1 Per-Channel Stage Detail (added 2026-07-14, simplified 2026-07-15)
...
Isinya adalah baris label:value singkat (pola `.kv` yang sama dengan panel
Power/Boot Health), satu baris per tahap — bukan kalimat panjang — supaya
bisa dipindai sekilas:

```text
Baseline       -0.000501 V (11 steps)
Exponential    range 255-511 (9 steps)
Binary search  code 444 (8 steps)
Confirm        code 443 @ 0.003419835 V (verified)
Final DAC      443
```

- **Baseline**: nilai baseline akhir + jumlah step (atau "scanning..." bila
  masih berjalan).
- **Exponential**: bracket `[low, high]` yang ditemukan + jumlah step, atau
  "failed" bila gagal.
- **Binary search**: code terpilih + jumlah step penyempitan.
- **Confirm**: code+voltage terpilih dengan tag singkat `verified`/`fallback`
  (dari mode firmware `baseline_threshold_verified`/`positive_verified`/
  `fallback_above_min`), atau "failed"/"checking...".
- Baris "Failed at" muncul bila channel gagal, isi ringkas: nama tahap +
  alasan singkat dari `NULLING_FAIL_REASON_TEXT`.
- **Final DAC**: baris terakhir, warna amber, kode DAC final + catatan
  `(+N bump)` bila ada penyesuaian setelah confirm.

Versi sebelumnya (2026-07-14) menulis satu kalimat panjang per tahap
(mis. "Exponential: searched from baseline X V to target Y V (threshold Z V)
in N steps..."); user meminta disederhanakan karena terasa berantakan dan
susah dipindai. Semua data source tetap sama, diparse dari field yang sudah
dikirim firmware (`GldNullingService.cpp`), hanya cara menampilkannya yang
dipadatkan. Mock GLD tidak mengirim semua field, jadi field yang hilang tetap
tampil sebagai `?` saat mode mock.
