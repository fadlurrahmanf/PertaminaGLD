// Small DOM helpers: badges, boot-log terminal, banner, tabs, drawers, and
// the local PIN lock that gates firmware upload.
...
export function setDrawer(which, open) {
  const panel = elements.setupPanel;
  const backdrop = elements.setupBackdrop;
...
// ---- Local PIN lock (deterrent only, mirrors ch-operator's ch-ui.js) ------
const PIN_KEY = "gwOperatorWeb.pinHash";
...
export function applyExpertLockUi() {
  ...
  if (elements.fwEnvSelect) elements.fwEnvSelect.disabled = locked;
  ...
  if (elements.expertLockNote) {
    elements.expertLockNote.textContent = locked
      ? "Locked. Click Unlock and enter the PIN to flash firmware."
      : "Unlocked. Choose a verified package folder and flash over USB.";
  }
}
