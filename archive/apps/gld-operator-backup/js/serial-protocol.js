...
// Sends a command, waits for its ack, and pops up a window.alert with the
// outcome so an Apply button always tells the operator whether it actually
// landed on the device (accepted, rejected by firmware validation, or no
// response at all) instead of leaving that only in the scrolling log.
export async function applyAndAlert(command, ackCmd, actionLabel) {
  try {
    const ack = await sendCommandAndWaitAck(command, ackCmd);
    if (ack.status === "ok") {
      window.alert(`${actionLabel}: berhasil diterapkan.${ack.message ? `\n${ack.message}` : ""}`);
    } else {
      window.alert(`${actionLabel}: DITOLAK oleh perangkat.\n${ack.message || ack.status}`);
    }
    return ack;
  } catch (error) {
    window.alert(`${actionLabel}: GAGAL - ${error.message}`);
    return null;
  }
}
