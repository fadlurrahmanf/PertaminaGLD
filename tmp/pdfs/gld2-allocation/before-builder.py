...
def end():
 txt(1035,765,f'{number:02d} / 17',10,COL['muted'])
...
export()
exec(compile((HERE/'diagram_pages.py').read_text(encoding='utf8'),str(HERE/'diagram_pages.py'),'exec'))
P.save()
...
titles=['Architecture','Main power','Always On','Sensor module','Analog acquisition','I2C and sensors','ESP32 connections','Alarm and status','System flow','Voltage calculations','Eight MQ heaters','Digital loads','Analog and module loads','Rail budget','Engineering notes','Sources 1','Sources 2']
...
Open `index.html` to view the seventeen pages with page selection and zoom, or open `../../pdf/GLD2-Block-Diagram.pdf` for the vector PDF.
The SVG files can be edited with Inkscape, Illustrator, or a text editor. Drawing helpers are in `build_diagram.py`, page content in `diagram_pages.py`, and source calculations in `power_budget.py`.
...
Brown power connections use plain voltage/current labels. Currents without sufficient evidence are marked "Current not determined"; equations and unresolved variables remain in the calculation appendix. Reference estimates are not measured consumption or guaranteed hardware maxima. power-calculations.json and power-sources.json provide reproducible values and traceable sources.
...
The generator outputs the PDF, seventeen SVG files, HTML, CSV, and README. Its primary input is the extracted `source-net-evidence.json`.
