...
**Status dokumen ini:** `design_update.md` adalah salinan kerja dari baseline immutable `design.md`, dipakai untuk menampung semua penyesuaian yang muncul saat implementasi (lihat `docs/design/gld/README.md`). Diperbarui terakhir 2026-07-01 mengikuti audit terhadap `firmware/gld/` dan schematic `docs/design/gld/schematics/gld_schematic_v2.json`. Perubahan ditandai inline dengan blockquote `>` di dekat bagian terkait; teks baseline asli tidak dihapus supaya histori tetap terlacak.
...
| Spreading Factor | `7` | `SF7` (diverifikasi terhadap firmware `GldUnifiedMain.cpp`/`GldInferenceMain.cpp` 2026-07-01; nilai `9` di baseline tidak lagi dipakai) |
| Coding Rate | `5` | coding rate yang benar-benar dipakai firmware (baseline sebelumnya menyebut `7`) |
...
| Flow | Inti langkah | Contoh call |
|---|---|---|
| Startup LoRa | Mulai SPI, inisialisasi radio dengan frekuensi / bandwidth / SF / CR target | `radio.begin(id.loraFreq, 125.0, 7, 5, 0x12, 17, 8);` (SF/CR diverifikasi terhadap firmware 2026-07-01) |
...
#### 4.1.5 TPL5010 + SN74AUP1G74 (power latch)
Source: BaseDesign, diverifikasi ulang terhadap schematic `docs/design/gld/schematics/gld_schematic_v2.json` dan firmware pada 2026-07-01.

> **Koreksi chip**: IC yang benar-benar dipasang adalah **TPL5010DDCR**, bukan TPL5110 seperti disebut baseline sebelumnya. Power gating juga tidak dilakukan sendirian oleh TPL5010 — ada flip-flop D **SN74AUP1G74DCUR** yang berfungsi sebagai power latch: pin `WAKE` milik TPL5010 men-set latch ini untuk menyalakan ESP32.

- Peran: timer wake + watchdog keepalive untuk battery mode (TPL5010), dikombinasikan dengan power latch (SN74AUP1G74) yang benar-benar memutus/menyambung rail daya ESP32
- Interface utama: `GPIO(DONE keepalive = GPIO14)`, `GPIO(CLR power-off = GPIO38)`
- Fungsi operasional: TPL5010 membangunkan node secara periodik lewat `WAKE` yang men-set latch `SN74AUP1G74`; firmware WAJIB mengirim pulsa keepalive ke `DONE` (GPIO14) setiap 10 detik selama masih bekerja, atau TPL5010 akan men-trigger `RSTn` dan me-reset paksa ESP32; mematikan daya yang sesungguhnya dilakukan lewat pin terpisah (`CLR`, GPIO38) yang meng-clear latch tersebut.
- Alur kerja firmware (2026-07-01):

| Flow | Inti langkah | Contoh call |
|---|---|---|
| Wake by timer | TPL5010 men-set latch `SN74AUP1G74` lewat pin `WAKE`, menyalakan rail battery path, node mulai boot | `battery wake cycle started by hardware timer + power latch` |
| Keepalive periodik | Selama node bekerja di battery mode, firmware memulai pulsa `DONE` setiap `GLD_TPL5010_KEEPALIVE_INTERVAL_MS` (10000 ms) agar TPL5010 tidak men-trigger `RSTn` | `pulseGldTpl5010Keepalive();` -> `digitalWrite(PIN_TPL5110_DONE, HIGH); delay(5); digitalWrite(PIN_TPL5110_DONE, LOW);` |
| Run one-shot task | Firmware menjalankan siklus singkat battery mode: scan sensor, inference, LoRa TX, receive window | `runScan(); transmitOnce();` |
| Clear latch (power off) | Jika tidak ada alarm aktif setelah siklus selesai, firmware men-clear latch lewat pin `CLR` (GPIO38, active-low) untuk benar-benar memutus daya ESP32 | `pulseGldPowerLatchClear();` -> `digitalWrite(PIN_POWER_LATCH_CLR, HIGH); delay(5); digitalWrite(PIN_POWER_LATCH_CLR, LOW); delay(5); digitalWrite(PIN_POWER_LATCH_CLR, HIGH);` |
| Alarm active -> stay awake | Jika alarm masih aktif, firmware TIDAK meng-clear latch (lampu alarm/buzzer digerakkan langsung oleh GPIO ESP32, bukan lewat latch terpisah, sehingga akan ikut mati kalau daya diputus); node tetap menyala dan mengecek ulang tiap siklus scan berikutnya | - |

Urutan kejadian battery mode:

1. TPL5010 berada pada kondisi sleep dan node tidak aktif (latch dalam keadaan clear).
2. Saat timer hardware mencapai interval wake, TPL5010 men-set latch lewat `WAKE`, rail menyala, node mulai boot.
3. ESP32 menjalankan siklus kerja (scan, inference, LoRa TX, receive window), sambil terus mengirim keepalive `DONE` setiap 10 detik.
4. Jika tidak ada alarm aktif, firmware meng-clear latch lewat pin `CLR` (GPIO38) — node benar-benar kehilangan daya di titik ini.
5. Jika alarm aktif, node tetap menyala (tidak meng-clear latch) sampai alarm mereda pada siklus scan berikutnya.

Catatan penting: pin `CLR` (GPIO38) ini **sama** dengan mekanisme "clear latched alarm error" pada tombol manual (lihat §3.18) — dipakai bersama untuk dua tujuan: clear alarm ter-latch secara manual, dan mematikan daya di akhir siklus battery-mode secara otomatis oleh firmware.
...
| Confirmation below/above offset | `5` (diverifikasi terhadap `GldNullingService.cpp` 2026-07-01: `CONFIRM_WINDOW=10` berarti ±5 kode DAC, bukan ±1 seperti baseline sebelumnya) |
| Confirmation sample count | `10` (diverifikasi terhadap `GldNullingService.cpp` 2026-07-01: `CONFIRM_COUNT=10`, bukan `5`) |
...
> Diimplementasikan 2026-07-01 di `GldUnifiedMain.cpp`/`GldNullingRuntimeMain.cpp`:
> boot flow ke mode `DATASET`/`NULLING` diblokir (kembali ke `INFERENCE` +
> restart) jika power bukan external (`battery` mode); mode `NULLING` juga
> diblokir jika alarm sebelumnya masih ter-latch (`readGldAlarmLatched()`,
> NVS namespace `gld` key `alarm`, di-set setiap kali `updateAlarmOutputs()`
> mengubah state alarm). `START_DATASET` juga menolak dengan
> `reject_battery_mode` jika power bukan external. "Nulling blocked during
> active dataset recording" belum relevan secara langsung karena arsitektur
> firmware memakai satu mode aktif per boot (tidak ada nulling dan dataset
> berjalan bersamaan dalam proses yang sama).
...
AI output class count  : 5-6 (clearGas, LPG, methane, propane, butane, anomaly fallback)
AI confidence threshold: 0.70 (alarm rule verified 2026-07-01: confidence >= 30% is the value firmware actually uses, see §7.2/alarm rule)
AI low confidence warn : 0.40
Initial tensor arena   : 8 KB
```

> Diverifikasi terhadap firmware 2026-07-01 (`GldUnifiedMain.cpp`, `model/model_data.cpp`): jumlah kelas output nyata adalah 5-6, bukan 9 seperti baseline sebelumnya.
...
> Diverifikasi 2026-07-01 terhadap `GldUnifiedMain.cpp`: alarm rule firmware
> nyata adalah `gasClass != clearGas && confidence >= 30` (`GLD_LEL_THRESHOLD_PERCENT = 30`),
> dipakai baik untuk alarm lokal maupun auto-transmit LoRa (tidak dipisah
> seperti di desain dasar). Firmware juga memiliki jalur klasifikasi gas
> tambahan non-ML, `GldThresholdClassifier` (deteksi LPG/Methane dari delta
> tegangan sederhana terhadap baseline), berjalan berdampingan dengan pipeline
> TFLite Micro tapi belum dipakai untuk keputusan alarm di `GldUnifiedMain.cpp`
> (alarm tetap mengikuti hasil TFLite). Belum pernah didokumentasikan di
> baseline sebelumnya.
...
- Diverifikasi 2026-07-01: firmware sebelumnya sempat punya array remap `HW_TO_MODEL` di `GldUnifiedMain.cpp`/`GldInferenceMain.cpp` yang melanggar aturan ini; array tersebut sudah dihapus sehingga channel hardware `n` kini benar-benar dipakai langsung sebagai feature index `n` (scaler/model harus dilatih dengan urutan yang sama, tidak di-remap)
...
> Diverifikasi 2026-07-01: firmware (`GldUnifiedMain.cpp`, `GldInferenceMain.cpp`, dan seluruh self-test main) memakai **Base radio profile (SF7/CR5)** secara seragam, bukan Field reference (SF9/CR7).
...
> Diimplementasikan 2026-07-01 di `GldPower.h`/`GldPower.cpp` dengan nilai
> yang sama persis (`GLD_BATTERY_MIN_VALID_VOLTAGE=3.00f`,
> `GLD_BATTERY_LOW_VOLTAGE=3.50f`, `GLD_BATTERY_CRITICAL_VOLTAGE=3.30f`,
> `GLD_BATTERY_FILTER_ALPHA=0.20f`), plus EMA filter dan field
> `batteryLow`/`batteryCritical` pada `GldPowerReading`, diekspos di
> `GLD_STATUS_JSON` sebagai `power.batteryLow`/`power.batteryCritical`.
> Sebelumnya tidak diimplementasikan sama sekali walau sudah tercatat di
> baseline sejak awal.
...
> **Status implementasi (diverifikasi 2026-07-01)**: fitur ini BELUM
> diimplementasikan sama sekali di `firmware/gld/` — tidak ada driver RS485,
> tidak ada register map, dan tidak ada pin RS485 (`GPIO19/20/21`) di
> `BoardPins.h`. Seluruh isi §12 di bawah ini masih mencerminkan desain yang
> diinginkan, bukan kondisi firmware saat ini. Perlu keputusan eksplisit:
> implementasikan, atau hapus dari scope.
...
> Diverifikasi 2026-07-01: firmware mengimplementasikan versi yang lebih ringan
> dari struct ini sebagai `RuntimeConfig` (NVS namespace `gld_app`, ditulis
> lewat command `SET_APP_CONFIG_JSON`), dengan field: `ctrlword` (harus
> `0xA3`, otomatis diisi saat `SET_APP_CONFIG_JSON` berhasil disimpan),
> `deviceId`, `nodeId`, `wifiSsid`, `wifiPassword`, `mqttHost`, `mqttPort`,
> `mqttUser`, `mqttPass`, `topicRoot`. Field `modelFilePath`, `defaultWiperArr`,
> `modbusSlaveId`, dan `loraFreq` **tidak ada** di struct firmware, karena:
> - model AI di-compile langsung ke firmware (`model/model_data.cpp`), bukan
>   dimuat dari file di LittleFS;
> - `wiperArr` hasil nulling disimpan lewat mekanisme `GldNullingProfile`
>   terpisah (NVS namespace `gld-null`), bukan sebagai default di
>   `RuntimeConfig`;
> - Modbus belum diimplementasikan sama sekali (lihat §1.1/§10);
> - `loraFreq` masih compile-time constant di `firmware/config/LoraStarConfig.h`.
> Validitas config diekspos lewat `runtimeConfigValid()` (`ctrlword==0xA3` dan
> `wifiSsid`/`mqttHost` tidak kosong), muncul di `GLD_INFO_JSON` sebagai
...
> Diverifikasi 2026-07-01: `firmware/gld/` TIDAK memakai struktur bertingkat
> di atas. Struktur nyata: `include/GldXxx.h` + `src/GldXxx.cpp` flat per modul
> (`GldAds1256Reader`, `GldDacMux`, `GldPower`, `GldMovingAverage`,
> `GldThresholdClassifier`, `GldNullingService`, `GldFrameBuilder`,
> `GldCommandParser`, `GldModeManager`, dll), ditambah entry point per mode
> (`GldUnifiedMain.cpp` untuk build aktif; `GldInferenceMain.cpp`,
> `GldDatasetMain.cpp`, `GldNullingRuntimeMain.cpp`, dan beberapa
> `*SelfTestMain.cpp` untuk build terpisah/self-test yang tidak ikut
> dikompilasi di env `gld` aktif). "Battery inference is one-shot and ends
> with TPL5110 DONE" juga sudah tidak akurat — lihat §4.1.5 untuk arsitektur
> TPL5010 + latch `SN74AUP1G74` yang benar.
...
Konstanta komunikasi yang dipasang pada firmware saat ini:
...
- firmware belum memverifikasi ACK secara final
- loop retry saat ini hanya mengulang bila `startTransmit()` gagal memulai transmisi
- jumlah percobaan maksimum = `1 initial try + 3 retry start attempts`
```

Pada battery mode, setelah `transmit`, node membuka `receive window` singkat default `2000 ms` sebelum sleep. Packet yang masuk pada window ini sudah bisa dibaca dan di-log, tetapi firmware saat ini belum melakukan matching ACK terhadap `seq`, belum menetapkan `ackStatus`, dan belum menjalankan command service dari LoRa downlink.


### 16.5 Scheduling Policy

Pada firmware saat ini, external mode memang memakai periode inferensi dan periode normal telemetry yang berbeda.

```text
External mode
- inference periodik setiap `10 s`
- normal telemetry LoRa ditargetkan per `60 s`
...
- setelah transmit, buka receive window default `2000 ms`
- packet masuk pada window dapat dibaca, tetapi belum diproses sebagai ACK final atau command executable
- setelah receive window selesai, firmware mem-pulse `DONE` ke `TPL5110`
```
...
- kirim segera
- target desain masih mengarah ke skema ACK, tetapi implementasi saat ini belum mewajibkan ACK final
- retry saat ini hanya terjadi bila start transmisi gagal, bukan karena ACK timeout
